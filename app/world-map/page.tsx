...
export default function WorldMapPage() {
  ...
  const fetchUnlockedCountries = async () => {
    try {
      // For now, we'll just use local state since we removed the backend
      // You can add persistence later (localStorage, database, etc.)
      console.log('Unlocked countries loaded from state')
    } catch (error) {
  ...
  const handleImageSelect = async (file: File) => {
    ...
    try {
      ...
      reader.onloadend = async () => {
        ...
        const response = await fetch('/api/dishes/recognize', {
          method: 'POST',
          headers: {
